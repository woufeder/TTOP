...
const articleLineCopy = computed(() => messages.value.library[articleLine.value])

const allArticles = computed(() => getArticles(locale.value, { line: articleLine.value }))
...
const filteredArticles = computed(() => {
  ...
  return allArticles.value.filter((article) => article.tags?.includes(activeTag.value))
})

const heading = computed(() =>
  activeTag.value ? `${articleLineCopy.value.tagPrefix}${activeTag.value}` : articleLineCopy.value.heading,
)
...
const detailRouteName = computed(() => (articleLine.value === 'story' ? 'story-detail' : 'article-detail'))
const isStoryLine = computed(() => articleLine.value === 'story')
const tagCounts = computed(() =>
  tags.value.map((tag) => ({
    tag,
    count: allArticles.value.filter((article) => article.tags?.includes(tag)).length,
  })),
)
...
<template>
  <AppShell>
      ...
      <div class="grid min-h-0 gap-6 xl:grid-cols-[minmax(0,1fr)_19rem]">
        <div class="min-h-0 rounded-[4px] border border-white/10 bg-[rgba(7,10,24,0.42)] px-6 py-6 text-parchment shadow-[0_24px_60px_rgba(0,0,0,0.22)] backdrop-blur-sm sm:px-8">
          <header class="border-b border-dashed border-white/12 pb-5">
            <p class="text-xs uppercase tracking-[0.42em] text-parchment/55">{{ articleLineCopy.eyebrow }}</p>
            <h1 class="mt-3 font-display text-4xl text-parchment">{{ heading }}</h1>
            <p class="mt-4 max-w-3xl text-sm leading-8 text-parchment/72">{{ articleLineCopy.description }}</p>
          </header>

          <div v-if="filteredArticles.length" class="divide-y divide-dashed divide-white/12">
            <article v-for="article in filteredArticles" :key="article.slug" class="py-8 first:pt-7">
              ...
              <RouterLink
                :to="localeRoute(detailRouteName, { slug: article.slug })"
              ...
              >
                {{ articleLineCopy.read }}
              </RouterLink>
            </article>
          </div>

          <section v-else class="py-8 text-center">
            <p class="font-display text-3xl text-parchment">{{ articleLineCopy.emptyTitle }}</p>
            <p class="mt-3 text-sm leading-8 text-parchment/68">{{ articleLineCopy.emptyDescription }}</p>
          </section>
        </div>
        ...
        <aside class="space-y-6">
          <section class="rounded-[4px] border border-white/10 bg-[rgba(7,10,24,0.5)] px-5 py-5 text-parchment shadow-[0_24px_50px_rgba(0,0,0,0.18)] backdrop-blur-sm">
            <p class="text-xs uppercase tracking-[0.42em] text-parchment/55">{{ articleLineCopy.sidebarTitle }}</p>
            <div class="mt-4 space-y-3">
              ...
              >
                <span>{{ articleLineCopy.all }}</span>
                <span class="text-parchment/55">({{ allArticles.length }})</span>
              </RouterLink>

              <RouterLink
                v-for="item in tagCounts"
                :key="item.tag"
                :to="localeRoute(tagRouteName, { tag: item.tag })"
    ...
    <ParchmentPanel
      v-else
      variant="wide"
      :eyebrow="articleLineCopy.eyebrow"
      :title="heading"
      :description="articleLineCopy.description"
    >
      ...
      <section v-if="!filteredArticles.length" class="paper-card mt-6 rounded-[24px] p-6 text-center">
        <p class="font-display text-3xl text-ink">{{ articleLineCopy.emptyTitle }}</p>
        <p class="mt-3 text-sm leading-7 text-ink/72">{{ articleLineCopy.emptyDescription }}</p>
      </section>
    </ParchmentPanel>
  </AppShell>
</template>